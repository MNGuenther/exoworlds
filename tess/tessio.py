...
#::: TESSIO
###############################################################################
def get(tic_id, sectors=None, server='pdo', pipeline='spoc', keys=None, PDC=False, auto_correct_dil=False, flatten=False, mask_flags=True, mask_nan=False):
    '''
    tic_id : str
    ...
    if pipeline=='spoc':    
        ...
        if len(fnames)>0:
            data = return_SPOC_data(fnames, keys=keys, PDC=PDC, auto_correct_dil=auto_correct_dil, flatten=flatten, mask_flags=mask_flags, mask_nan=mask_nan)
            return data
...
#::: TESSIO CSV
###############################################################################
def csv(tic_id, sectors=None, server='pdo', pipeline='spoc', keys=None, PDC=False, auto_correct_dil=False, flatten=False, outfilename=None, mask_flags=True, mask_nan=False):
    tic_id = str(int(tic_id))
    data = get(tic_id, sectors=sectors, server=server, pipeline=pipeline, keys=keys, PDC=PDC, auto_correct_dil=auto_correct_dil, flatten=flatten, mask_flags=mask_flags, mask_nan=mask_nan)
    if outfilename is None:
        home = os.path.expanduser("~")
